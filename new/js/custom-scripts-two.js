    ...
    {   
        ...
        var init = function() {
            ...
            btn_shuffle.click(function() {
                ...
                $(this).addClass('shuffling').css('display','none').fadeOut('slow');
                instructions.find('.date,.text-begin').fadeOut();

                instructions.find('.reshuffle-info').fadeOut().html('<p class="text-shadow-minimal lt-orange italic shuffling-cards">SHUFFLING</p>').fadeIn().fadeOut().fadeIn().fadeOut().fadeIn();

                $($('.card').get().reverse()).each(function(){
                    var card = $(this);
                    setTimeout(function(){
                        
                        card.animate({ 'margin-left' : '60px', 'margin-top': '-25px' },1);
                        setTimeout(function(){
            ...
            var reshuffleCard = function(cards, i, animate) {
                setTimeout(function() {
                    ...
                    cards.removeClass('shuffled');
                    btn_shuffle.removeClass('shuffling').css('visibility','visible');
                    instructions.find('.reshuffle-info').fadeOut().html('<p class="text-shadow-minimal">All Done!</span><br class="visible-xs">').css('color','#00A65A').fadeIn().delay(150);
                    instructions.find('.text-begin, .date').fadeIn();
                    $(".instructions").css("display", "none");
                    deal(card_rows.first());
                },animate ? options.shufflenext : 2000); 
            }
        ...
        var cardClick = function(obj) {
            ...
            if (csa == cis) {
                ...
                $('.nav-guide').addClass('collapse').fadeIn('slow');
            ...
            // good to select card
                ...
                var settop = parseInt(parseInt(top)-parseInt(0));
                var margintop = parseInt(parseInt(top)-parseInt(20));

                if ($(window).width() > 767){

                    setTimeout(function(){
                        card.animate({ 'margin-top' : settop },1).unbind('click').removeClass('on').addClass('card-selected').attr('data-selected',true).attr('data-position',csr+1);
                        $('.card-selected').attr('style', function(i,s) { return (s || '') + 'transition-duration: 0.5s !important' });
                    },50);

                } else {

                    setTimeout(function(){
                        card.animate({ 'margin-top' : margintop },1).unbind('click').removeClass('on').attr('data-selected',true).attr('data-position',csr+1);
                    },50);
                ...
                if (cis - csel > 0) {
                    ...
                    if(cis-csel==2)
                    {
                        content='<b>Select Card Two</b>: How They Feel About You'; 
                    }
                     else if(cis-csel==1)
                    {
                        content='<b>Select Card Three</b>: What They’ll Do Next';    
                    }
                    else
                    { }
                    ...
                    $('.nav-guide').addClass('collapse').fadeIn('slow');
                    // $('#love-reading').removeClass('none').css('display','none').fadeIn('slow');
                    // window.setTimeout(function() {
                    //     window.location.href = 'https://readyourlover.com/new/landing.php#love-reading';
                    // }, 1000);
                    guide.fadeOut('1', function() {
                        btn_get_reading.parent().hide().removeClass('collapse').fadeIn('slow');
                        $('.nav-guide').addClass('collapse').css('display','none').fadeIn('slow');
        ...
        var deal = function(row) {
            if (row.length > 0) {
                ...
                // spread the deck
                var cardpc = eval(options.cardWidth) / container.width() * 75; // percentage of width that one card takes on the screen  (+10 correction)
                // var cardpc = eval(options.cardWidth+10) / container.width() * 100;
        ...
        var moveCard = function(cards, seq, q, count, animate) {
            ...
            if (seq < count) {
                if (count > 38 && seq == count/2) {
                    ...
                    $('.choose-wrap .row .col-md-3').css('width', '100%');
                    $('.choose-wrap .row .col-md-9').css('width', '100%');
                    $('.limit-cards').addClass('limit-cards-remove');
                    ...
                    if ($(window).width() > 767){
                        ...
                        if (animate) {
                            ...
                            $(".cards-row").css("margin-bottom", "5px");
    
                            window.setTimeout(function(){
                                guide.hide().removeClass('collapse').fadeIn(1000);
                                // $(".container").css("padding-bottom", "100px").fadeIn(1000);
                            }, 800);
 
                        } else {
                            $('.card').addClass('instantly');
                            
                            // $(".instantly").animate({ 'margin': '0 12px 35px' }, 500);
                        ...
                        if (animate) {         
                            ...
                            $(".limit-cards").css("padding-left", "0").fadeIn(2000);

                            cards.eq(seq).delay(100).animate({'margin-left':q+'%'},options.dealmove,'linear',function() {
                                moveCard(cards, seq+1, q, count, animate);
                            });
