...
(function($){
    ...
    {   
        ...
        var defaults = {
            cardWidth: 144,     // width of the card (px)
            ...
            //bpXS: 639,          // XS breakpoint
            animate: function() { return true; } // animations enabled?
        };
        ...
        var btn_box_begin   = $('.text-begin');

        var sub_navbar      = $('.sub-navbar');
        var guide           = $('.guide');
        var cards_remain    = $('.cards-remaining');
        var card_animate    = $('.animation-switch');
        ...
        // Override some variables for xs view
        // enquire.register("screen and (max-width:"+options.bpXS+"px)", {
        //     match : function() {
        //         card_animate = $('.animation-switch-xs');
        //         zen_mode_switch     = $('.zen-mode-xs');
        //         zen_toggle   = $('.zen-toggle-xs');
        //     },
        // });

        var init = function() {
            // Switch animations off on small devices
            enquire.register("(max-width:767px)", {
                match : function() {
                    if (card_animate.find('[name="card-animate"]').is(':checked'))  {
                        card_animate.find('[name="card-animate"]').bootstrapSwitch('toggleState');
                    }
                },
            });
            ...
            btn_shuffle.click(function() {
                ...
                $(this).addClass('shuffling').css('visibility','hidden');
                instructions.find('.date,.text-begin').fadeOut();

                instructions.find('.reshuffle-info').fadeOut().html('&nbsp;<br class="visible-xs"><span class="text-shadow-minimal lt-orange italic shuffling-cards">shuffling</span>').fadeIn().fadeOut().fadeIn().fadeOut().fadeIn();

                $($('.card').get().reverse()).each(function(){
                    var card = $(this);
                    setTimeout(function(){
                        
                        card.animate({ 'margin-left' : '30px', 'margin-top': '-15px' },1);
                        setTimeout(function(){
                ...
                reshuffleCard(card_rows.first().find('.card'), 0);

                stack_cards(0);

                setTimeout(function() {

                    $('#cards-row-1').css("cssText", "max-width:1000px;transition: max-width 1s ease;");
            ...
            var reshuffleCard = function(cards, i, animate) {
                setTimeout(function() {
                    ...
                    btn_shuffle.removeClass('shuffling').css('visibility','visible');
                    instructions.find('.reshuffle-info').fadeOut().html('&nbsp;<br class="visible-xs"><span class="text-shadow-minimal">All Done!</span><br class="visible-xs">').css('color','#00A65A').fadeIn().delay(150);
                    instructions.find('.text-begin, .date').fadeIn();
                    $(".instructions").css("display", "none");
                    deal(card_rows.first());
                },animate ? options.shufflenext : 3500); 
            }
            ...
            form.submit(function() {
                ...
                if (csel < cis) {
                    ...
                    if(cis-csel==3)
                    {
                        bootbox.alert('Choose <b>3</b> more cards');    
                    }
                    else if(cis-csel==2)
                    {
                        bootbox.alert('Choose <b>2</b> more cards');    
                    }
                     else if(cis-csel==1)
                    {
                        bootbox.alert('Choose <b>1</b> more card');    
                    }
                    else
                    { }
                    return false;
            ...
            card_color.find('a').click(function() {
                ...
                choose_cards.removeClass(card_color_curr.attr('color')).addClass($(this).attr('color'));
            ...
            zen_mode_switch.find('input').on('switchChange.bootstrapSwitch', function(event, state) {
                if (!state) {
                    ...
                    choose_cards.addClass("cards-row-bg");
                    ...
                    choose_outer.fadeOut('fast', function () {
                        ...
                        choose_cards.removeClass("cards-row-bg");
            ...
            $(btn_box_begin).find('span').click(function () { beginReading(); });
        }
        ...
        var beginReading = function(obj) {
            if (start_date.length > 0 && !start_date.val()) {
                bootbox.alert('Before you begin please select '+start_date.attr('placeholder'));
                return false;
            }
            if (!btn_shuffle.hasClass('shuffling')) {
                card_rows.first().find('.card').last().unbind('click'); // unbind begin function from last card
                instructions.removeClass('visible-xs').fadeOut();
                choose_cards.removeClass('cards-row-bg');
                deal(card_rows.first());
            }
        }
        ...
        var cardClick = function(obj) {
            ...
            if (csa == cis) {
             ...
             //bootbox.alert('You\'ve already picked ' + cis + ' card' + (cis > 1 ? 's' : '') + '');
               
                var inst = $('[data-remodal-id=getreading]').remodal();
            ...
            // good to select card
                ...
                var settop = parseInt(parseInt(top)-parseInt(20));
            
                setTimeout(function(){
                    card.animate({ 'margin-top' : settop },1).unbind('click').removeClass('on').attr('data-selected',true).attr('data-position',csr+1);
                ...
                if (cis - csel > 0) {
                    ...
                    if(cis-csel==2)
                    {
                        // content='Choose <b>2</b> more cards';  
                        // content='<b>Select Card 2</b>: How They Feel About You';  
                        content=' Select your <b>Second card</b>…'; 
                    }
                     else if(cis-csel==1)
                    {
                        // content='Choose <b>1</b> more card';  
                        // content='<b>Select Card 3</b>: What They’ll Do Next';
                        content='Select your <b>Third card</b>… ';    
                    }
                    else
                    { }

                    guide.find('p').html(content);
                } else {
                    // setTimeout(function() {
                    //     var inst = $('[data-remodal-id=getreading]').remodal();
                    //     inst.open();
                    ...
                    //     enquire.register("(max-height:767px)", {
                    //         match : function() {
                    ...
                    $(".choose-cards .row.nav-guide").fadeOut("slow");

                    setTimeout(function() { 
                        // $(".choose-cards .container-progress-bar").css("cssText", "display:block !important;").fadeIn("slow");
                        $(".choose-cards .container-progress-bar").css("cssText", "display:block;").addClass("fadeIn");

                        $('html, body').animate({
                            scrollTop: $(".choose-cards .container-progress-bar").offset().top
                        }, 500);
                    ...
                    $(function () {
                        ...
                        function checkValue(obj) {
                            ...
                            switch (true) {
                                case (val <= 16):
                                    $("#label_changing h2").html("Creating sacred space…");
                                    break;
                                case (val > 16 && val <= 32):
                                    $("#label_changing h2").html("Laying card spread…");
                                    break;
                                case (val > 32 && val <= 48):
                                    $("#label_changing h2").html("Invoking spirit guides…");
                                    break;
                                case (val > 48 && val <= 64):
                                    $("#label_changing h2").html("Connecting with the Universe…");
                                    break;
                                case (val > 64 && val <= 80):
                                    $("#label_changing h2").html("Receiving insights…");
                                    break;
                                case (val > 80 && val <= 100):
                                    $("#label_changing h2").html("Preparing answers…");
                                    hideProgressBar();
                                    break;
                            }
                    ...
                    var hideProgressBar = function(){
                        setTimeout(function() { 
                            $(".choose-cards .container-progress-bar").fadeOut("slow").removeClass("fadeIn").addClass("fadeOut");
                        }, 1500);
                        // showNewForm();
                    ...
                    $('html, body').animate({
                        scrollTop: $(".choose-cards").offset().top
                    }, 500);
                    ...
                    setTimeout(function() { 
                        $(".get-my-reading-wrap").css("cssText", "display:block;").addClass("fadeIn");

                    }, 5500);

                    var showNewForm = function(){
                        // setTimeout(function() { 
                        //     $(".container-newform").css("cssText", "padding: 25px; background: #fff; border-radius: 15px; max-width: 800px; margin: auto; display:block !important;").addClass("fadeInDown");
                        // }, 2000);
                        ...
                        guide.fadeOut('1', function() {
                            ...
                            enquire.register("(max-height:767px)", {
                                match : function() {
                                    $('html, body').animate({scrollTop: $('.btn-get-reading').offset().top - 200}, 3400);
                                },
                            });
                        });
                    ...
                    // $('#cards-row-1').css("cssText", "max-width:500px;padding-top:20px;");
                    // $('.selected_card').addClass('selected_card_style');
                    // $('.selected_card').css("cssText", "margin:-20px 2% 0;");
                }
            }
            // already selected enought cards in a row
            else {
                bootbox.alert('You\'ve already picked ' + cisr + ' card' + (cisr > 1 ? 's' : '') + (card_rows.length > 1 ? ' in this row!' : '!'), function(result){
                    /* your callback code */ 
                })
        ...
        var deal = function(row) {
            if (row.length > 0) {
                ...
                moveCard(cards, 1, q, cardco, options.animate.call());
            }
        ...
        var moveCard = function(cards, seq, q, count, animate) {
            ...
            //     var percents = 0;
            //     stack_cards(0);
            //     $($('.card').get().reverse()).not(':last').each(function(){
            //         var card = $(this);
            //         // $('.card').addClass('blip');
            //         setTimeout(function(){
            //             diff +=35;
            //             // diffe = card.css('width', ($(this).width() * 100) + '%');
            //             card.animate({ 'margin-left' : '3.5%' });
            //             i++;
            //         },time);
            //         time += 2;
            //         counter++; 
